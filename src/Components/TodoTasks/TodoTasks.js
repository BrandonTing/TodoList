...
const todoTasks = (props) => {
  // 利用filter排除掉正在執行的任務。
  const tasks = props.tasks.filter(task => props.tasks.indexOf(task) !== props.currentTask);
  // 只顯示前三個待辦事項。
  const top3Tasks = tasks.slice(0, 3);
  const taskLists = top3Tasks.map(task => (
    <CSSTransition
      key = {task.name}
      classNames={{
  ...
  const more = props.tasks.length > 4 ? <a href='#fullList' className = {classes.More}>...More</a> : null;
  return (
